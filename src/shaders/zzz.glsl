...
//    float hue = fract(polar.phi / twoPi) * 360.;
    ...
    vec2 p = abs(uv);
    // r = max(p.x, p.y);
    alpha *= gauss(r, 0.65, 0.11);
    // alpha *= step(0., polar.phi);
    alpha *= smoothstep(0., 0.2, polar.phi) * smoothstep(pi, pi - 0.2, polar.phi);
    hue = smoothstep(0.65, 0.1, r) * 360.;

    // or deactivate?
    alpha *= periodicGauss(polar.phi, peakAngle, widthAngle);
    hue = smoothstep(0.8, 0.3, polar.r) * 360.;
...
#define DO_SOME_COORDINATE_TRANSFORMATION_FIRST 0

void drawSomethingOnTop(inout vec3 col, in vec2 uv) {
    vec3 colBG = col;

    /// wie bisher - erst Koordinatentransformation nach Bedarf
    #if DO_SOME_COORDINATE_TRANSFORMATION_FIRST
        uv.y -= 0.25;
    ...
    if (transformHSV) {
        hsv = rgb2hsv(col);
        // hsv.x = fract(polar.r - iTime) * 360.;
        // hsv.x = mod(hsv.x - 180. * alpha, 360.);
        // hsv.y *= mix(1., 0., alpha);
        hsv.y = 0.5 + 0.5 * sin(twoPi * (polar.r - iTime));
        col = hsv2rgb(hsv);
    }
    if (transformOKLCh) {
        LCh oklch = rgb2oklch(col);
        oklch.hue = fract(polar.r - iTime) * twoPi;
        col = oklch2rgb(oklch);
    }
    */
}
